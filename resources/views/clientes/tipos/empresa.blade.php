<div class="card mb-4" id="form-container">
    ...
    <div class="card-body">
        <form id="cliente-form" action="{{ route('clientes.store') }}" method="POST">
            @csrf
            <input type="hidden" name="tipo_cliente" value="2"> <!-- 2 = Empresa -->
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    ...
                    <small id="ejemploDocumento" class="form-text text-muted"></small>
                </div>
                <div class="col-md-4">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    <label for="cod_actividad_economica" class="form-label">Giro</label>
                    <select class="form-control" id="cod_actividad_economica" name="cod_actividad_economica">
                        <option value="">Seleccione giro</option>
                        @foreach($actividadesEconomicas as $actividad)
                            <option value="{{ $actividad->id_actividad_economica }}">
                                {{ $actividad->cod_actividad_economica }} - {{ $actividad->actividad_economica }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <!-- Tipo Contribuyente Section -->
            <div class="row mb-3">
                <div class="col-md-4">
                    <label for="fk_id_tipo_contribuyente" class="form-label">Tipo contribuyente</label>
                    <select class="form-control" id="fk_id_tipo_contribuyente" name="fk_id_tipo_contribuyente">
                        <option value="">Seleccione tipo</option>
                        @foreach($tiposContribuyente as $tipo)
                            <option value="{{ $tipo->id_tipo_contribuyente }}">{{ $tipo->tipo_contribuyente }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
...
$(document).ready(function() {
    ...
    $('#cod_tipo_documento').change(function() {
        ...
        const ejemploDocumento = $('#ejemploDocumento');
        
        if (tipoDocumento === '2') { // DUI
            $('#dui_nit').attr('placeholder', '12345678-9');
            ejemploDocumento.text('Ejemplo: 12345678-9');
        } else if (tipoDocumento === '3') { // DUI Homologado
            $('#dui_nit').attr('placeholder', '123456789');
            ejemploDocumento.text('Ejemplo: 123456789');
        } else if (tipoDocumento === '1') { // NIT
            $('#dui_nit').attr('placeholder', '0614-010101-101-4');
            ejemploDocumento.text('Ejemplo: 0614-010101-101-4 (ingrese sin guiones)');
        } else {
            $('#dui_nit').attr('placeholder', '');
            ejemploDocumento.text('');
        }
    });

    // Disparar el evento al cargar si ya hay un valor seleccionado
